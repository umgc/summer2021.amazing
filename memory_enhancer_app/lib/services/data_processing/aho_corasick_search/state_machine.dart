//**************************************************************
// State Machine for Aho-Corasick algorithm
...
class StateMachineState<State, Input> {
  ...
  late final StateMachine<State, Input> stateMachine;
  ...
  int _stateIndex(State state) => stateMachine.stateIndexMap[state]!.toInt();
...
class StateMachine<State, Input> {
  ...
  void _initializeStateIndexMap() {
    ...
    //if (stateIndexMap != null && stateIndexMap.isNotEmpty) {
    if (stateIndexMap.isNotEmpty) {
      return;
    }
    stateIndexMap = {};
