//**************************************************************
// Aho-Corasick algorithm for finding words in a text
...
/// Pattern Matching algorithm that searches for all occurrences of
...
class AhoCorasick {
  ...
  /// words are either at the beginning / end or are surrounded by spaces
  ...
  factory AhoCorasick.fromWordList(List<String> patterns,
    ...
    return AhoCorasick._(
        ...
        stateMachine: StateMachine<WordState, String>(
            ...
            transition: ({required WordState activeState, required String input}) {
  ...
  Match _fixShiftedMatch(Match match) => separateBySpaces && match != null
      ? Match(startIndex: match.startIndex, word: match.word.substring(1, match.word.length - 1))
      : match;
  ...
  Match? firstMatch(String input, {bool longest = false}) {
    ...
    if (longest) {
      Match? m = _firstMatchLongest(fullInput);
      if(m != null) {
        return _fixShiftedMatch(m);
      }
    }
    final state = stateMachine.createState();
